import argparse
import threading
from fast import load_exploits, run_exploit
...
def main():
    ...
    invalid_names = [n for n in args.names if n not in [e.name for e in selected_exploits]]
    if invalid_names:
        logger.error(f"Exploits with the following names not found: {st.bold(', '.join(invalid_names))}")
        logger.info(f"Available exploits: {st.bold(', '.join([e.name for e in exploits]))}")
